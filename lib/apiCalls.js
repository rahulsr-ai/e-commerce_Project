...
export const getUserOrder = async () => {
  try {
    const { data } = await axios.get(`/api/orders/getUserOrder`);
    return data;
  } catch (error) {
    console.log("frontend error while fetching wishlist", error);
    return null;  
...
export const getAllOrdersData = async () => { 
  try {
     const {data} = await axios.get(`/api/orders/AllOrderData`);
     return data;
  } catch (error) {
     console.log("frontend error while fetching wishlist", error);
     return null;
...
export const updateUserOrderStatus = async (orderId, status) => {
  try {
    const { data } = await axios.post(`/api/orders/updateOrderStatus`, {
      orderId,
      status,
    });
    return data;
  } catch (error) {
    console.log("frontend error while updating user order status", error);
    return null;
  }
};


export const findUserSearch = async (val) => {
  try {
    const res = await axios.get(`/api/search?find=${val}`);
    console.log(res.data?.products);
    return res.data?.products
  } catch (error) {
    console.log("frontend error while finding user search", error);
    return null;
  }
};
